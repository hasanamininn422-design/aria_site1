...
این مرحله هنوز برای عرضه عمومی توصیه نمی‌شود: بازیابی رمز و تأیید ایمیل، CMS، آپلود امن و صفحات حقوقی نهایی لازم‌اند. ورود، نشست و مدیریت درخواست‌ها روی PostgreSQL محلی تست شده‌اند؛ راهنمای مدیر در docs/ACCOUNTS.md است. قبل از انتشار، پایش، rate limit در ورودی شبکه، سقف اندازه درخواست در reverse proxy و TLS را تنظیم کنید.
